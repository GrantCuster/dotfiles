...
# Modified to use dither and diff mode rectangle
...
ffmpeg -v warning -i $input_path -vf "$filters,palettegen=stats_mode=diff" -y $palette
if [[ -f $palette ]]; then
  ffmpeg -v warning -i $input_path -i $palette -lavfi "$filters [x]; [x][1:v] paletteuse=dither=bayer:diff_mode=rectangle" -y $output_path
  rm $palette
fi
